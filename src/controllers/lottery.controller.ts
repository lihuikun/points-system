...
export class LotteryController {
  ...
  static drawLottery: RequestHandler = async (req, res) => {
    try {
      ...
      console.log("🚀 ~ LotteryController ~ drawLottery:RequestHandler= ~ user:", userId)
      ...
      res.json(ResponseHandler.success(lotteryResult));
    } catch (error) {
  ...
  private static getLotteryResult() {
    ...
    if (random < 0.01) {
      ...
      return { name: '八等奖', value: 0 };
    } else {
      return { name: '九等奖', value: Math.floor(Math.random() * 200) + 1 };
    }
  }
